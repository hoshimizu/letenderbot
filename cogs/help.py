...
class Help(commands.Cog):
    ...
    async def help(self, interaction: discord.Interaction):
        ...
        help_embed.add_field(name='/ping', value='ボットの応答速度を表示します。', inline=False)
        help_embed.add_field(name='/serverinfo', value='サーバーの情報を表示します。', inline=False)
        help_embed.add_field(name='/userinfo', value='ユーザーの情報を表示します。', inline=False)
        help_embed.add_field(name='/poll', value='投票を作成します。', inline=False)
        help_embed.add_field(name='/createinvite', value='招待リンクを作成します。', inline=False)
        help_embed.add_field(name='/say', value='ボットに発言させます（管理者のみ）。', inline=False)
